...
class UnblockAccountServiceTest {
    ...
    @InjectMocks
    private UnblockAccountService service;


    @Test
    public void unblockAccountOk() throws AccountException {
        ...
        verify(baasIntegration, times(1)).updateAccount(any(), eq(AccountStatusEnum.ACTIVE));
        verify(postMessageService, times(1)).executeAsync(any());

        assertEquals("Account has been unlocked successfully.", response.getMessage());
    }

    @Test
    public void unblockAccountNotFoundShouldThrowResourceNotFound() throws AccountException {

        when(accountRepository.findById(any())).thenReturn(Optional.empty());

        var response = assertThrows(ResourceNotFoundException.class, () -> service.execute(1l));

        verify(baasIntegration, times(0)).updateAccount(any(), eq(AccountStatusEnum.ACTIVE));
    ...
    @Test
    public void unblockAccountStatusInvalidShouldThrowBadRequestException() throws AccountException {
